...
pub fn get_subtitle_font(config: &Ini) -> Font {
    load_ttf_font_from_bytes(&get_font_from_bytes(config, "subtitle_font"),
    )
    .unwrap()
}
...
fn get_font_from_bytes(config: &Ini, key: &str) -> Vec<u8> {
    get_bytes(get_data_directory().join(get_font_section(config).get(key).unwrap()).to_str().unwrap())
}