...
use common::{PanelType, State, Paths};
...
impl IO {
    ...
    pub fn update(
        ...
        let undo = match state.panels[state.focus.get()] {
            PanelType::Music => self.music_panel.update(state, conn, input, tts, text, paths),
            PanelType::Tracks => self.tracks_panel.update(state, conn, input, tts, text, paths),
