...
pub(crate) fn get_edit_mode_status_tts(mode: &EditMode, text: &Text) -> String {
    text.get_with_values("PIANO_ROLL_PANEL_STATUS_TTS_EDIT_MODE", &[&text.get_edit_mode(mode)])
}

pub(crate) fn get_cycle_edit_mode_input_tts(mode: &Index, input: &Input, text: &Text) -> String {
    let mut m1 = mode.clone();
    m1.increment(true);
    let index = m1.get();
    get_tooltip_with_values("PIANO_ROLL_PANEL_INPUT_TTS_EDIT_MODE", &[InputEvent::PianoRollCycleMode], &[&text.get_edit_mode(&EDIT_MODES[index])], input, text)
}
