...
class UiField(ABC):
    def __init__(self, title: str, font_color: Color):
        self._title: str = title
        self._title_color: Color = font_color

    @final
    def render(self, position: Tuple[int, int], focus: bool, vertical: bool) -> List[Command]:
        commands = [Text(text=self._title,
                         text_color=COLORS[Color.panel_background] if focus else COLORS[self._title_color],
                         background_color=COLORS[self._title_color] if focus else COLORS[Color.panel_background],
                         position=position,
