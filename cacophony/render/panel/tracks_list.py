    ...
    def _do_result(self, result: RenderResult) -> bool:
        ...
        if did:
            self.affected_panels.update({PanelType.piano_roll: {"track_index", self.selection_index},
                                         ...
                                         PanelType.new_track: {"current_track_index": self.selection_index}})
            return True
        # Show the new track panel.
        elif InputKey.add in result.inputs_pressed:
            self.active = False
            self.affected_panels.update({PanelType.new_track: {"active": True}})
            return True
        return False
